def gcp_files_to_email(bucket_name, bucket_folder, emails, email_subject):
    ...
    try:
        ...
        # Zipeamos los CSVs
        with zipfile.ZipFile(f'{base_path}/{bucket_folder}/CSV_files.zip', mode='w') as z:
            for file in os.listdir(f'{base_path}/{bucket_folder}'):
                if os.path.splitext(file)[1] == '.csv':
                    z.write(f'{base_path}/{bucket_folder}/{file}',
                            basename(f'{file}'))

            if os.stat(f'{base_path}/{bucket_folder}/CSV_files.zip').st_size == 0:
                os.remove(f'{base_path}/{bucket_folder}/CSV_files.zip')

            elif os.stat(f'{base_path}/{bucket_folder}/CSV_files.zip').st_size != 0:
                print("OK LOCAL SAVE: 'CSV_files.zip'")
    ...
    if exists(f'{base_path}/{bucket_folder}'):
        ...
        sender_email = Variable.get('sender_email', deserialize_json=True)
        message = MIMEMultipart()
        message['from'] = sender_email['email_address']
        ...
        with smtplib.SMTP(host='smtp.gmail.com', port=587) as smtp:
            smtp.ehlo()
            smtp.starttls()
            smtp.login(sender_email['email_address'],
...
def gcp_files_to_email_operator(task_id, bucket_name, bucket_folder, emails, email_subject):
    """
    Returns an Airflow PythonOperator using the function 'gcp_files_to_email'
    ...
    return PythonOperator(
        task_id=task_id,
        python_callable=gcp_files_to_email,
        op_kwargs=dict(
